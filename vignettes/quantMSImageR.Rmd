...
| `contribution_hm()` | the same arguments | the same `ComplexHeatmap::Heatmap`, with group mean as hue and per-sample contribution as opacity |
...
`contribution_hm()` draws the same matrix but encodes two things per cell: the
...
It takes the same arguments as `quantile_hm()` and returns the same kind of
...
contribution_hm(combined, quant_val = 0.5,
                heatmap_order = sections, heatmap_labs = groups,
                feature_split = feature_split, feature_split_name = "Met-1",
                cell_size = 10, fontsize = 7)
```
