...
labelled: airways, parenchyma, a vessel. `labelROIs()` shows the ion image of a
selected transition, across tissue pixels only. Then draw a region, name it,
and choose whether to add another:
...
When any acquisition has regions, the report adds a section comparing them:

- a map of the regions in every sample;
- box plots for each feature, arranged by `compare`:
  ...
  - `"both"` -- both sets;
- a heatmap section, one row per sample and region, split by region, with
  **each region z-scored against itself** -- so a colour says where a sample
  sits among the other samples' airways, rather than restating that airways
  and parenchyma differ (`heatmap: False` leaves it out);
- an `ROI_summary` sheet in the tables workbook: one row per point (see
...
roi:
  ...
  heatmap:            True              # the by-region heatmap section | False
  unit:               "sample"          # sample | roi | both
```
