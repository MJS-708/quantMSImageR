...
test_that("plotCalCoverage returns a two-panel plot for all calibrated features", {
  obj <- calibrated_obj()
  p   <- plotCalCoverage(obj, val_slot = "intensity")
...
test_that("features can be selected by name or by index", {
  ...
  by_name  <- plotCalCoverage(obj, features = nm, val_slot = "intensity")
  by_index <- plotCalCoverage(obj, features = 1, val_slot = "intensity")
...
test_that("standards are taken from the model frame, matching the fitted line", {
  ...
  obj <- calibrated_obj()
  nm  <- fData(obj)$name[1]
  p   <- plotCalCoverage(obj, features = nm, val_slot = "intensity")
...
test_that("missing calibration or concentration slot fails informatively", {
  ...
  expect_error(plotCalCoverage(raw), "createCalCurve")

  # Models fitted, but int2conc() not run
  cal_metadata <- read.csv(file.path(cal_dir, "calibration_metadata.csv"))
  fitted <- summariseCalLevels(raw, cal_metadata, val_slot = "intensity",
                                 cal_label = "Cal", id = "identifier")
  fitted <- createCalCurve(fitted, cal_type = "cal")
  expect_error(plotCalCoverage(fitted, val_slot = "intensity"), "int2conc")
})

test_that("an uncalibrated feature name is rejected with the available list", {
  obj <- calibrated_obj()
  expect_error(
    plotCalCoverage(obj, features = "not_an_analyte", val_slot = "intensity"),
