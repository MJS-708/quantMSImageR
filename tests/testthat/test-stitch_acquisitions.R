...
test_that("stitching is the union of pixels under one run", {
  ...
  a <- o[, 1:50]
  b <- o[, 51:n]
  w <- stitch_acquisitions(a, b, label = "whole")
...
test_that("a list is accepted, as the YAML path supplies one", {
  o <- obj()
  w <- stitch_acquisitions(list(o[, 1:50], o[, 51:ncol(o)]), label = "whole")
...
test_that("overlapping pieces are refused, not silently merged", {
  ...
  o <- obj()
  expect_error(stitch_acquisitions(o[, 1:50], o[, 40:60], label = "whole"),
...
test_that("pieces must share features and layers", {
  ...
  expect_error(stitch_acquisitions(a, b, label = "whole"),
               regexp = "different features")
})

test_that("it refuses to stitch a single acquisition", {
  expect_error(stitch_acquisitions(obj(), label = "whole"),
               regexp = "at least two")
})

test_that("stitch differs from bind_panels on non-overlapping halves", {
  # bind_panels() intersects pixels, so on two halves that do not overlap it
  # has nothing to keep. That is the mistake stitch_acquisitions() exists to
  # prevent, and the YAML `combine:` key exists to choose between.
  o <- obj()
  a <- o[, 1:50]
  b <- o[, 51:ncol(o)]
  expect_error(bind_panels(a, b, label = "whole"),
               regexp = "no shared")
  expect_s4_class(stitch_acquisitions(a, b, label = "whole"),
                  "MSImagingExperiment")
})
