# combineMSIs() cbinds acquisitions. Cardinal's own failure for mismatched
...
test_that("mismatched features name the offending object", {
  expect_error(combineMSIs(mk("s1"), mk("s2", names = c("f1", "f9"))),
               regexp = "object 2 has different features")
})

test_that("mismatched spectra layers are caught before cbind", {
  expect_error(combineMSIs(mk("s1"), mk("s2", layers = "snr")),
...
test_that("duplicate run identifiers are rejected", {
  ...
  expect_error(combineMSIs(mk("s1"), mk("s1")),
