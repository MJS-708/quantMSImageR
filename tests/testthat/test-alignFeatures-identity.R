# alignFeatures() rewrites obj2's mz key so Cardinal's cbind will accept it.
...
test_that("a shared name over a different transition is refused", {
  ...
  expect_error(alignFeatures(a, b),
               regexp = "share a name but not a transition")
  # The message must name the offender and show both transitions.
  expect_error(alignFeatures(a, b), regexp = "f2: 355.2 -> 275.1 vs 355.2 -> 300")

  # Opting out is possible, but has to be said.
  expect_silent(al <- alignFeatures(a, b, feature_match = "name"))
  expect_equal(mz(al$obj2), mz(al$obj1))
})
...
test_that("agreement is judged at unit resolution", {
  ...
  expect_silent(alignFeatures(a, b))
  ...
  expect_silent(alignFeatures(a, c2))
...
test_that("objects without transition metadata must opt in to name matching", {
  ...
  expect_error(alignFeatures(bare("s1"), bare("s2")),
               regexp = "needs precursor_mz and product_mz")
  expect_silent(alignFeatures(bare("s1"), bare("s2"), feature_match = "name"))
})

test_that("alignFeatures rejects non-MSI input with a named error", {
  expect_error(alignFeatures(data.frame(), data.frame()),
               "must both be MSImagingExperiment")
})
