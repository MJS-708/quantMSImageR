...
#' @param snr_thresh Numeric. SNR threshold passed to [generateTxtImages()].
#'   Default `3`.
...
#'   bundled synthetic calibration standards (`summariseCalLevels()` ->
#'   `createCalCurve()` -> `int2conc()`)? In a real study this step is driven
#'   by the YAML `calibration:` block. Default `TRUE`.
#'
#' @return Invisibly returns the list produced by [generateTxtImages()], with
...
#' @family workflow
#' @export
runExample <- function(render_report = TRUE,
                        output_txt    = FALSE,
                        snr_thresh    = 3,
                        shapes        = NULL,
                        calibrate     = TRUE) {
  ...
  message("In a real study you would run selectTissuePixels() before the")
  ...
  # standards acquisition via readMRM); the synthetic standards ship as an RDS,
  ...
  if (calibrate) {
    ...
    if (file.exists(cal_rds)) {
      ...
      cal_obj <- summariseCalLevels(cal_obj, cal_meta, val_slot = "intensity",
                                      cal_label = "Cal", id = "identifier")
      cal_obj <- createCalCurve(cal_obj, cal_type = "cal")

      # Apply the curves to the STUDY sections, exactly as runStudy.R does:
  ...
  if (render_report) {
    ...
    # general threshold, with the IS override (half) applied. Mirrors runStudy.R.
    ...
    feature_meta      <- buildFeatureMeta(combined, ion_lib_meta)
