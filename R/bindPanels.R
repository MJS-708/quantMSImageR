...
#'   MRM selects Q1 and Q3 at unit resolution). See [readMRM()].
#'
...
#' the same steps.
#'
#' @seealso [combineMSIs()], [generateTxtImages()]
#'
#' @examples
...
#' @export
bindPanels <- function(obj1, obj2, label = NULL,
  ...
  if (!is(obj1, "MSImagingExperiment") || !is(obj2, "MSImagingExperiment"))
    stop("bindPanels: obj1 and obj2 must both be MSImagingExperiment ",
  ...
  if (!all(c("x", "y") %in% names(pd1)) ||
    ...
    stop("bindPanels: both inputs must carry x and y columns in pData().")
  ...
  if (length(common) == 0)
    stop("bindPanels: no shared (x, y) coordinates between the two objects.")

  n1 <- length(k1); n2 <- length(k2); nC <- length(common)
  if (nC < n1 || nC < n2)
    message(sprintf(
      "  bindPanels: keeping %d pixels (intersection); dropping %d from obj1 and %d from obj2.",
      nC, n1 - nC, n2 - nC))
  ...
  if (length(overlap) > 0) {
    ...
    if (feature_match == "transition") {
      ...
      if (!all(need %in% names(f1)) || !all(need %in% names(f2)))
        stop("bindPanels: feature_match = \"transition\" needs precursor_mz ",
      ...
      if (length(bad))
        stop("bindPanels: ", length(bad), " feature name(s) appear in both ",
...
Keeping one would discard a different measurement. Rename them ",
    ...
    message(sprintf(
      "  bindPanels: dropping %d duplicate feature(s) from obj2: %s",
  ...
  if (!identical(sort(lyr1), sort(lyr2)))
    stop("bindPanels: the two objects carry different spectra layers (",
