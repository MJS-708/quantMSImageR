...
setMethod("summarise_cal_levels", "quant_MSImagingExperiment",
          function(MSIobject, cal_metadata, val_slot = "intensity", cal_header = "sample_type", cal_label = "Cal", id = "identifier"){
            ...
            if(length(.miss))
              stop("summarise_cal_levels: cal_metadata is missing column(s): ",
            ...
            if(length(.dup)){
              ...
              if(nrow(unique(cbind(key = .key[.key %in% .dup], .cmp))) !=
                 length(unique(.dup)))
                stop("summarise_cal_levels: cal_metadata has duplicated ",
            ...
            if(!cal_header %in% names(pixel_data))
              stop("summarise_cal_levels: pData() has no column '", cal_header,
                   "'. Columns present: ",
            ...
            if(nrow(pixel_data) == 0)
              stop("summarise_cal_levels: no pixels are labelled '", cal_label,
            ...
            if(length(.orphan))
              stop("summarise_cal_levels: cal_metadata names analyte(s) that ",
            ...
            for(i in seq_len(.n)){
              ...
              if(length(pixels) == 0)
                stop("summarise_cal_levels: calibration spot '", cal_n,
