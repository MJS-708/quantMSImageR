...
#'
#' @return Invisibly, the list produced by [generateTxtImages()] for the
...
runStudy <- function(config_file) {
  ...
  cal_val  <- .cal$val_slot         %||% (if (.use_is) "response" else "intensity")
    ...
    cal_obj <- readMRM(name = .cal$cal_acquisition, folder = data_path,
  ...
  if (.use_is && identical(cal_val, "response"))
    ...
    cal_obj <- summariseCalLevels(cal_obj, cal_metadata, val_slot = cal_val,
                                    cal_label = "Cal", id = "identifier")
    cal_obj <- createCalCurve(cal_obj, cal_type = cal_type,
