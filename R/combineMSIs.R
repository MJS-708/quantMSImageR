setGeneric("combineMSIs", function(MSIobject, ...) standardGeneric("combineMSIs"))
...
#' @aliases combineMSIs
#' @export
setMethod("combineMSIs", "MSImagingExperiment",
          function(MSIobject, ...){
            ...
            for(ind in seq_along(objects)[-1]){
              if(!identical(.nm(objects[[ind]]), .nm(objects[[1]])))
                stop("combineMSIs: object ", ind, " has different features ",
                     "from object 1. Align them with alignFeatures() first, ",
                     "after checking they really are the same transitions.",
                     call. = FALSE)
              if(!identical(.lay(objects[[ind]]), .lay(objects[[1]])))
                stop("combineMSIs: object ", ind, " has spectra layers (",
            ...
            if(anyDuplicated(.runs))
              stop("combineMSIs: run identifiers must be unique across ",
