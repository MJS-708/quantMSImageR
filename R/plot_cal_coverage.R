setGeneric("plot_cal_coverage", function(MSIobject, ...) standardGeneric("plot_cal_coverage"))
...
#'
#' @family calibration
#' @aliases plot_cal_coverage
#' @export
setMethod("plot_cal_coverage", "quant_MSImagingExperiment",
          function(MSIobject, features = NULL, val_slot = "intensity",
            ...
            if(length(.cs) == 0)
              stop("plot_cal_coverage: no '", conc_slot, "' slot. Run int2conc() ",
                   "first. Slots present: ", paste(slots_present, collapse = ", "),
                   call. = FALSE)
            conc_slot = .cs[1]
            if(!val_slot %in% slots_present)
              stop("plot_cal_coverage: no '", val_slot, "' slot. Slots present: ",
            ...
            if(is.null(features)){
              ...
              if(length(unknown) > 0)
                stop("plot_cal_coverage: no calibration model for feature(s) ",
                     paste(unknown, collapse = ", "), ". Calibrated features: ",
                     paste(available, collapse = ", "), call. = FALSE)
            }

            if(length(features) == 0)
              stop("plot_cal_coverage: none of the object's features have a ",
            ...
            for(nm in features){
              ...
              if(!any(keep)){
                warning("plot_cal_coverage: no positive finite pixels for '", nm,
                        "'; skipping.", call. = FALSE)
                next
              }
            ...
            if(length(px_l) == 0)
              stop("plot_cal_coverage: no feature had usable pixel values.",
                   call. = FALSE)
